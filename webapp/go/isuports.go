...
func competitionScoreHandler(c echo.Context) error {
	...
	_, err = tenantDB.Exec("INSERT INTO player_score (id, tenant_id, player_id, competition_id, score, row_num, created_at, updated_at) VALUES "+strings.Join(placeholders, ","), values...)
	if err != nil {
		return fmt.Errorf(
...
func playerHandler(c echo.Context) error {
	...
	for _, c := range cs {
		...
		psM := playerScoresMap.Get(strconv.Itoa(int(v.tenantID)) + "-" + c.ID)
		if psM != nil {
